...
export function allocStringArray(mem, malloc, list) {
    ...
    const arrayPtr = malloc(ptrs.length * 4);  // Memory for storing the array of pointers
    const dv = new DataView(mem.buffer, arrayPtr, ptrs.length * 4);

    // Ensure each pointer in the array is correctly set
    ptrs.forEach((p, i) => {
        dv.setUint32(i * 4, p, true);  // Set each string pointer in the array
    });
